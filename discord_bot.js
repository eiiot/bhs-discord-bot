...
client.on("ready", () => {
  ...
  async function verifyMessage() {
    const channel = client.guilds.cache
      ...
      .channels.cache.get(`787039874384396329`);
...
client.on("messageCreate", async (message) => {
  ...
  if (
    messageContent[0].toLowerCase() === ".deploy_slash_command" &&
  ...
  if (
    messageContent[0].toLowerCase() === ".slash_command_id" &&
    message.author.id === client.application?.owner.id
  ) {
    if (messageContent.length < 2) {
      message.reply(
        "Please use the following format: `.slash_command_id <command name>`"
      );
      return;
    }
    ...
    const command = commandArray.find(
      (command) => command[1].name === messageContent[1]
    );
  ...
  if (
    messageContent[0].toLowerCase() === ".slash_command_delete" &&
    message.author.id === client.application?.owner.id
  ) {
    if (messageContent.length < 2) {
      message.reply(
        "Please use the following format: `.slash_command_delete <command name>`"
      );
      return;
    }
  ...
  if (
    messageContent[0].toLowerCase() === ".react_with_heart" &&
    message.author.id === client.application?.owner.id
  ) {
    message.delete();
  ...
  if (
    message.content.startsWith(".slash_command_perms") &&
    message.author.id === client.application?.owner.id
  ) {
    ...
    if (args.length < 4) {
      message.reply(
        "Please use the following format: `.slash_command_perms <command id> <true/false> <role id>`"
      );
      return;
    }
  ...
  if (
    messageContent[0].toLowerCase() === "ping" &&
    message.author.id === client.application?.owner.id
  ) {
...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isCommand()) {
    if (interaction.commandName === "user") {
      ...
      // load database
      const emailsDatabase = JSON.parse(
      ...
      if (action === "unverify") {
        if (member !== undefined) {
          if (
            !member.roles.cache.some((role) => role.id === "762720121205555220")
          ) {
            if (
              !member.roles.cache.some(
                (role) => role.id === "765670230747381790"
              )
            ) {
              const embed = {
                color: 0xeff624,
      ...
      if (action === "resetnick") {
        ...
        const embed = {
          ...
          description: `<@${user.id}>'s nickname has been reset.`,
          timestamp: new Date(),
        };
        await interaction.reply({
          embeds: [embed],
    ...
    if (interaction.commandName === "suggest") {
      ...
      suggestionMsg.react(
        interaction.guild.emojis.cache.get("879376341613568040")
      );
      suggestionMsg.react(
        interaction.guild.emojis.cache.get("879376341630341150")
      );
    ...
    if (interaction.commandName === "leaderboard") {
      axios
        .get(
          "https://mee6.xyz/api/plugins/levels/leaderboard/762412666521124866"
        )
...
expressApp.get("/discord", async (req, res) => {
  try {
    ...
    const discordToken = await axios.post(
    ...
    for (var i = 0; i < emailsDatabase.length; i++) {
      if (
        emailsDatabase[i].email === userEmail ||
        emailsDatabase[i].id == discordUserID
      ) {
        if (
          emailsDatabase[i].version >= 2.0 ||
          emailsDatabase[i].id !== discordUserID
        ) {
