...
client.on('messageCreate', async message => {
  ...
  if (messageContent[0].toLowerCase() === '.studyroom' && message.author.id === client.application?.owner.id) {
    message.reply(`My commands have moved! Try /studyroom`);
	}

  if (messageContent[0].toLowerCase() === '.help' && message.author.id === client.application?.owner.id) {
    message.reply(`My commands have moved! Try /help`);
	}
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'studyroom') {    
    ...
    if (users === undefined) {
      const voiceChannel = guild.channels.create(`${interaction.user.username}'s Study Room`, {
        type: 'GUILD_VOICE',
    ...
    const embed = {
      ...
      description: `Created the channel ${interaction.user.username}'s Study Room!`,
      timestamp: new Date(),
    };
